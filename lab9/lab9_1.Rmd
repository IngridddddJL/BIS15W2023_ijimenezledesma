...
```{r}
ggplot(data = homerange, mapping = aes(x=log10.hra, y=log10.preymass))+ geom_jitter(na.rm = T)+
   geom_smooth(method = lm, se=T)
```
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+ #x in this case has to be categorical 
  geom_col()
```
...
homerange %>% 
  filter(class=="mammalia") %>% 
  count(trophic.guild) %>% 
  ggplot(aes(x=trophic.guild, y=n))+ geom_bar()
```
