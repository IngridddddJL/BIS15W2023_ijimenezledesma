...
colleges %>%
  group_by(city) %>% 
  summarise(m_cost= mean(costt4_a, na.rm = T)) %>% 
  arrange(desc(m_cost))
```
...
colleges %>% 
  ...
  arrange(desc(m_cost)) %>% 
  slice_head(n=10) %>% 
  ggplot(aes(x=city, y=m_cost))+geom_col()
```
...
```r
ggplot(data = colleges, mapping= aes(x=adm_rate, y=c150_4_pooled))+geom_point()+geom_smooth(method = lm, se = T)
```
...
```r
ggplot(data = colleges, mapping= aes(x=costt4_a, y=c150_4_pooled))+geom_point()+geom_smooth(method = lm, se = T)
```
...
```r
univ_calif_final %>% 
  ggplot(aes(x=instnm, y=adm_rate)) + geom_col() + coord_flip()
```
...
```r
univ_calif_final %>% 
  ggplot(aes(x=instnm, y=pcip26)) + geom_col() + coord_flip()
```
